...
########################################
# Global parameters
# cor.type <- c("weighted", "unweighted", "identity")[1]
# use.pt.mass <- T
# omegaseq <- 1 * exp(-3:3 * log(2))
# M <- length(omegaseq)
replace.files <- T
for(dataset in c("impc", "eqtl")){
  if(dataset == "impc"){
    load(file = uv.results.Y.S)
    Pseqsub <- Pseq[Pseq <= ncol(Yhat)]
    Nseqsub <- Nseq[Nseq <= sum(linemap$line.type == "trueMut") / 2]
    P.full <- 148
  }
  if(dataset == "eqtl"){
    ...
    Pseqsub <- Pseq[Pseq <= ncol(mash.in$strong.z)]
    Nseqsub <- Nseq[Nseq <= nrow(mash.in$random.z) / 2]
    P.full <- 44
  }
  dir.dataset <- paste0(sub.data.sets.dir, "/", dataset)
  dir.create(dir.dataset, showWarnings = F)
  nseed.per.scenario <- 50
  for(N in Nseqsub){
    for(P in c(P.full, Pseqsub)){
      npdir <- paste0(dir.dataset, "/N_", N, "_P_", P)
      dir.create(npdir, showWarnings = F)
      for(seedc in 1:nseed.per.scenario){
        file.out <- paste0(npdir, "/", dataset, "_N_", N, "_P_", P, "_seed_", seedc, ".RData")
        if(!file.exists(file.out) | replace.files){
          if(dataset == "eqtl"){
          ...
          if(dataset == "impc"){
            ...
            refline.genos <- linemap[linemap$geno.nozyg %in% ref$genotype_id & linemap$geno %in% truemut.genzyg, "geno"]
            ...
            linemap[match(negcon.genzyg[1:N], linemap$geno), "line.type"] <- "negConTra"
            linemap[match(negcon.genzyg[(N + 1):n.null.lines], linemap$geno), "line.type"] <- "negConTes"
            n.true.lines <- length(truemut.genzyg)
            linemap[match(truemut.genzyg[1:N], linemap$geno), "line.type"] <- "trueMutTra"
            linemap[match(truemut.genzyg[(N + 1):n.true.lines], linemap$geno), "line.type"] <- "trueMutTes"
            linemap <- linemap[rowSums(is.na(linemap)) == 0, ]

            neg.tra.use <- linemap$geno[linemap$line.type == "negConTra"]
            true.tra.use <- linemap$geno[linemap$line.type == "trueMutTra"]
            neg.tes.use <- linemap$geno[linemap$line.type == "negConTes"]
            true.tes.use <- linemap$geno[linemap$line.type == "trueMutTes"]
            ...
            ph.use <- ph.use[order(phmap[match(ph.use, pout$ph), "procnam"])]
...
#   seedc <- as.numeric(arguments[grep("--args", arguments) + 1])
...
#   # seedc <- 7575
#   seedc <- as.numeric(format(Sys.time(), "%s")) %% 30000
#   do.plots <- T
#   full.analysis <- F
#   if(full.analysis){
#     N <- 1500
