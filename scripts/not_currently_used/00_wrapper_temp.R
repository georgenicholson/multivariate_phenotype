...
if (grepl("MVphen", Meth)) {
  ...
  if(run_type == "main"){
    ...
    saveRDS(object = loocv.store, file = loocv.res.store.namc)
  }
}
...
if(Meth %in% c("mash", "XD")){
  ...
  if(Meth == "XD"){
    ...
    if(!file.exists(XD.output.file.namc) | replace.XD){
      ...
      saveRDS(object = XD_result, file = XD.output.file.namc)
    ...
    saveRDS(object = resl.store, file = XD.resl.file.namc)
  }
  ...
  if(Meth == "mash"){
    ...
    saveRDS(object = resl.store, file = mash.resl.file.namc)
    mash_results <- list(res.mash.all.testing = res.mash.all.testing, res.mash.fitted.model = res.mash.fitted.model)
    saveRDS(object = mash_results, file = mash.raw.results.file.namc)
  }
}
