...
library(GO.db)
go.plot.dir <- "X:/projects/impc_mv_analysis/plots/impc_mv_paper_plots/annotation_plots"
dir.create(go.plot.dir, showWarnings = F)
...
resl.out[[fac.meth]]$signsig.bigeff <- (abs(resl.out[[fac.meth]]$t) > resl.out[[fac.meth]]$th & 
                                          abs(resl.out$uv$mn) >= mn.th.big) * sign(resl.out$uv$t)
...
genemap.in <- genemap[genemap$genotype_id %in% gene.all.num, ]
sym2eg <- toTable(org.Mm.egSYMBOL2EG)
...
egGenomicLoc <- toTable(org.Mm.egCHRLOC)
...
genfacl <- list()
for(restype in c("uv", "mv", "f", "mv.bigeff", "uv.bigeff")){#restype <- "uv"#
...
if(run.GO.analysis){
  ...
  outl <- foreach(phc = featvc, .verbose = T, .errorhandling = "pass") %dopar% {
    ...
    for(restype in restypev){
      ...
      set.seed(1)
      res_hyper <- go_enrich(input_hyper, n_randset = nperm, organismDb = 'Mus.musculus', 
  ...
  # goidvgoidv <- unlist(lapply(outl, function(x) c(x$uv$node_name, x$mv$node_name, x$mv.bigeff$node_name, x$uv.bigeff$node_name)))
  ...
  # uniquenams <- names(gotermtab)#[gotermtab <= Inf]
  # uniqueids <- names(goidtab)#[goidtab <= Inf]
  # outl.sub <- lapply(outl, function(x) list(uv.bigeff = x$uv[x$uv.bigeff$node_name %in% uniquenams, ], 
  #                                           mv.bigeff = x$mv.bigeff[x$mv.bigeff$node_name %in% uniquenams, ]))
  # outl.sub <- outl.sub[sapply(outl.sub, function(x) NROW(x$uv.bigeff) + NROW(x$mv.bigeff) > 0)]
  # outl.sub
  go.gene.dat <- get_anno_genes(go_ids = goidv, database = 'Mus.musculus', genes = NULL, annotations = NULL,
...
nperm <- 500
phc <- ph.use[1]#outl <- foreach(phc = featvc, .verbose = T, .errorhandling = "pass") %dopar% {#for(phc in phsub){#phc <- ph.use[20]#
  library(topGO)
  library(GOfuncR)
  library(org.Mm.eg.db)
  allresl <- list()
...
maptab <- maptab[maptab$agg > 0, ]
maptab <- maptab[order(-maptab$)]
