...
partc <- c("debug", "debug11", "debug6", "debug8", "debug4", "debug7")[6]
file.copy(from = "scripts/01_model_fitting_wrapper.R",
          to = "scripts/01_model_fitting_wrapper_temp.R", overwrite = T)
scen <- 3#for (scen in scen_to_run) {#scen <- 1#
subsamseed <- 1#for (subsamseed in 1:analysis_table$n_subsamples[scen]) {#subsamseed <- 1#
