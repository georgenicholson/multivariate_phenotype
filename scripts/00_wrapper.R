...
# for (scen in 6) {
  ...
  dput(names(analysis_table))
  for (var_curr in var_to_assign) {
    assign(x = var_curr, value = analysis_table[scen, var_curr])
  }
  ...
  for (subsamseed in 1:n_subsamples) {
    ...
    #Initialise R
    if(Data == "impc"){
      R.init <- cor((Y / S)[sams_for_cor_est, phens_to_use], use = "p", meth = "p")
    ...
    if(Data == "eqtl"){
      ...
      data.in <- list(Bhat = Y[sams_for_cor_est, phens_to_use], Shat = S[sams_for_cor_est, phens_to_use])
      R.init <- mashr::estimate_null_correlation_simple(data = data.in, z_thresh = 2)
      # Sig.init <- cov(Y[sams_for_model_training, phens_to_use], use = "p", meth = "p")
    }
    ...
    if (!grepl("rand", Meth)) {
      if(nSig > 1){
        require(mclust)
        mcl.out <- Mclust(data = Y[sams_for_model_training, phens_to_use], G = nSig)
      ...
      for(j in 1:nSig){
        ...
        Sig.init <- cov(Y[sams.in, phens_to_use, drop = F], use = "p", meth = "p")
    ...
    if (grepl("MVphen", Meth)) {
      ...
      if(run_type == "main"){
        ...
        for(procc in procun){
          ...
          post.mn.loocv <- em.update.function(Y.em = Yhat.left.out[sams_for_model_testing, phens_to_use], S.em = smat.left.out[sams_for_model_testing, phens_to_use],
    ...
    if(Meth %in% c("mash", "XD")){
      ...
      # bo <- run.XD
      XDmeth <- Meth
      file.base.mash <- paste0(control$methods_comp_dir, "/", paste(paste(control$variables_in_filename_mash, sapply(control$variables_in_filename_mash, get), sep = "_"), collapse = "_"))
      file.base.XD <- paste0(control$methods_comp_dir, "/", paste(paste(control$variables_in_filename_XD, sapply(control$variables_in_filename_XD, get), sep = "_"), collapse = "_"))
      ...
      if(Data == "impc"){
        ...
        uv.t.mat <- (Y / S)[sams_for_model_training, ]
        ...
        if(any(colMeans((Ztil == 0)) == 1)) #If phen's completely missing among big effects then use all samples
          Ztil <- scale((Y / S)[sams_for_model_training, phens_to_use], scale = F)
      }
      if(Data == "eqtl"){
      ...
      if(Meth == "mash"){
        ...
        for(mashmeth in mashmethv){#mashmeth <- mashmethv[[1]]#
            ...
            mashdata.all.testing <- mash_set_data(Bhat = Y[sams_for_model_testing, phens_to_use], Shat = S[sams_for_model_testing, phens_to_use], alpha = 0, V = R.init)
